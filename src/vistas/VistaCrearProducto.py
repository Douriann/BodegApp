...
class VistaCrearProducto(ctk.CTkToplevel):
    ...
    def _crear_formulario(self):
        scroll_frame = ctk.CTkScrollableFrame(self, width=380, height=500)
    ...
    def _guardar(self):
        try:
            ...
            if not nombre or nombre.isdigit():
            	raise ValueError("El nombre del producto no puede estar vacio ni ser solo numeros")

            if not presentacion or presentacion.isdigit():
            	raise ValueError("La presentacion no puede estar vacio ni ser solo numeros")

            if not unidad or unidad.isdigit():
